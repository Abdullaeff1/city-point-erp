...
| — | `Guest.fin_code` (indexed, normalized uppercase) |
...
1. Walk-in requires FIN, first/last name, company, ID held=True (override needs permission + reason).
...
6. FIN masked in UI unless `reception.view_sensitive_data`.
